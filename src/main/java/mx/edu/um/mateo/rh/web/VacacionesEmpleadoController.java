...
public class VacacionesEmpleadoController extends BaseController {
    ...
    public String lista(HttpServletRequest request, HttpServletResponse response,
        ...
        if (StringUtils.isNotBlank(correo)) {
            ...
            try {
                ...
                modelo.addAttribute(Constantes.CONTAINSKEY_MESSAGE_ATTRS, new String[]{messageSource.getMessage("vacacionesEmpleado.lista.label", null, request.getLocale()), ambiente.obtieneUsuario().getUsername()});
        ...
        List<VacacionesEmpleado> vacacionesEmpleados = (List<VacacionesEmpleado>) params.get(Constantes.CONTAINSKEY_VACACIONESEMPLEADO);
        Long primero = ((pagina - 1) * max) + 1;
        log.debug("primero {}", primero);
        log.debug("clave empleado size {}", vacacionesEmpleados.size());
        Long ultimo = primero + (vacacionesEmpleados.size() - 1);
    ...
    public String graba(HttpServletRequest request, HttpServletResponse response, @Valid VacacionesEmpleado vacacionesEmpleado,
        ...
        redirectAttributes.addFlashAttribute(Constantes.CONTAINSKEY_MESSAGE_ATTRS, new String[]{vacacionesEmpleado.getDescripcion()});
    ...
    public String elimina(HttpServletRequest request, @RequestParam Long id, Model modelo, @ModelAttribute VacacionesEmpleado vacacionesEmpleado,
            BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        log.debug("Elimina clave de empleado");
        try {
            manager.elimina(id);

            redirectAttributes.addFlashAttribute(Constantes.CONTAINSKEY_MESSAGE, "vacacionesEmpleado.elimina.message");
            redirectAttributes.addFlashAttribute(Constantes.CONTAINSKEY_MESSAGE_ATTRS, new String[]{vacacionesEmpleado.getDescripcion()});
        } catch (Exception e) {
            log.error("No se pudo eliminar la clave con id" + id, e);
            bindingResult.addError(new ObjectError(Constantes.ADDATTRIBUTE_VACACIONESEMPLEADO, new String[]{"vacacionesEmpleado.no.elimina.message"}, null, null));
            return Constantes.PATH_VACACIONESEMPLEADO_VER;
        }
    ...
    private void generaReporte(String tipo, List<VacacionesEmpleado> vacacionesEmpleados, HttpServletResponse response) throws JRException, IOException {
    ...
    private void enviaCorreo(String tipo, List<VacacionesEmpleado> vacacionesEmpleados, HttpServletRequest request) throws JRException, MessagingException {
