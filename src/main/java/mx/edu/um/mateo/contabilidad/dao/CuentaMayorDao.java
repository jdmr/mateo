...
public class CuentaMayorDao {
	...
	private static final Logger log = LoggerFactory
			.getLogger(CuentaMayorDao.class);
	@Autowired
	...
	public Map<String, Object> lista(Map<String, Object> params) {
		...
		if (!params.containsKey(Constantes.CONTAINSKEY_MAX)) {
			...
			params.put(Constantes.CONTAINSKEY_MAX, Math.min(
					(Integer) params.get(Constantes.CONTAINSKEY_MAX), 100));
		}

		if (params.containsKey(Constantes.CONTAINSKEY_PAGINA)) {
			Long pagina = (Long) params.get(Constantes.CONTAINSKEY_PAGINA);
			Long offset = (pagina - 1)
					* (Integer) params.get(Constantes.CONTAINSKEY_MAX);
		...
		if (params.containsKey(Constantes.CONTAINSKEY_ORGANIZACION)) {
			criteria.createCriteria(Constantes.CONTAINSKEY_ORGANIZACION).add(
					Restrictions.idEq(params
							.get(Constantes.CONTAINSKEY_ORGANIZACION)));
			countCriteria.createCriteria(Constantes.CONTAINSKEY_ORGANIZACION)
					.add(Restrictions.idEq(params
							.get(Constantes.CONTAINSKEY_ORGANIZACION)));
		}
		...
		if (params.containsKey(Constantes.CONTAINSKEY_ORDER)) {
			...
			if (params.get(Constantes.CONTAINSKEY_SORT).equals(
					Constantes.CONTAINSKEY_DESC)) {
		...
		if (!params.containsKey(Constantes.CONTAINSKEY_REPORTE)) {
			criteria.setFirstResult((Integer) params
					.get(Constantes.CONTAINSKEY_OFFSET));
			criteria.setMaxResults((Integer) params
					.get(Constantes.CONTAINSKEY_MAX));
		}
		params.put(Constantes.CONTAINSKEY_MAYORES, criteria.list());

		countCriteria.setProjection(Projections.rowCount());
		params.put(Constantes.CONTAINSKEY_CANTIDAD, (Long) countCriteria.list()
				.get(0));
	...
	public CuentaMayor obtiene(Long id) {
		...
		CuentaMayor cuentaMayor = (CuentaMayor) currentSession().get(
				CuentaMayor.class, id);
		return cuentaMayor;
	}
	...
	public CuentaMayor crea(CuentaMayor cuentaMayor, Usuario usuario) {
		...
		if (usuario != null) {
			cuentaMayor.getId().getEjercicio().getId()
					.setOrganizacion(usuario.getEmpresa().getOrganizacion());
		}
	...
	public CuentaMayor actualiza(CuentaMayor cuentaMayor, Usuario usuario) {
		...
		CuentaMayor nueva = (CuentaMayor) currentSession().get(
				CuentaMayor.class, cuentaMayor.getId());
		BeanUtils.copyProperties(cuentaMayor, nueva, new String[] { "id",
				"version" });

		if (usuario != null) {
			nueva.getId().getEjercicio().getId()
					.setOrganizacion(usuario.getEmpresa().getOrganizacion());
		}
