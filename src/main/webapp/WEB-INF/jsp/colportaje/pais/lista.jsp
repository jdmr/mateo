...
<html>
    <head>
        <title><s:message code="pais.lista.label" /></title>
    </head>
    <body>
        ...
        <h1><s:message code="pais.lista.label" /></h1>
        <hr/>

        <form name="filtraLista" class="form-search" method="post" action="<c:url value='/colportaje/pais' />">
                ...
                <tbody>
                    <c:forEach items="${paises}" var="pais" varStatus="status">
                        <tr class="${status.index % 2 == 0 ? 'even' : 'odd'}">
                            <td><a href="<c:url value='/colportaje/pais/ver/${pais.id}' />">${pais.nombre}</a></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
